...
if ($query) {
    ...
    $solr = new Apache_Solr_Service('localhost', 8983, '/solr/firstrank');
...
<body>
    ...
    <?php if ($results): ?>
        ...
        ?>
        <?php if ($corrector != ""): ?>
            <p>Do you want to search: <a href="http://localhost/572hw4/hw5.php?q=<?php echo $corrector; ?>"><?php echo $corrector; ?></a></p>
            <?php 
                $results = $solr->search($corrector, 0, $limit, $param);
                $total = (int)$results->response->numFound;
                $start = min(1, $total);
                $end = min($limit, $total);
            ?>
        ...
        <?php foreach ($results->response->docs as $doc): 
            $id = $doc->og_url;
            //echo $doc->id;
            $id = substr(strrchr($doc->id,"/"),1);
            //echo $id;
            //$url = urldecode($id);
            //echo $url;
            if(empty($new_query)) $new_query = $query;
            $html = file_get_html("ABCNews/".$id);
    if($html != false){
            $temp = $html->find('body',-1);
            if ($temp == null) continue;
            $content = $temp->plaintext;
            if ($content == null) continue;
            if(strlen($content) == 0) continue;
            
            //$pattern = '/\[body\](.*?)\[\/body\]/';
            //$pattern = "^\w{20}".$new_query."^";
            //$pattern = '/^[a-zA-Z0-9]{10,50}'.$new_query.'+.*/';
    /*
            ...
            //echo sizeof($matches);
            $content = $matches[0];
            $content = strrchr($content,"<body");
            echo $content;
    */
            $radius = 150;
            $ending = "...";
            if(empty($new_query)) $new_query = $query;
            if($corrector != "") $new_query = $corrector;
            $phraseLen = strlen($new_query); 
            if ($radius < $phraseLen) { 
                 $radius = $phraseLen; 
             } 

             $phrases = explode (' ',$new_query);
             $flag = false;
             foreach ($phrases as $new_query) {
                 $pos = strpos(strtolower($content), strtolower($new_query)); 
                 if ($pos > -1) { $flag = true; break; }
             }
             ...
             $excerpt = substr($content, $startPos, $endPos - $startPos); 
    ...
    ?>
            <?php 
                    $id = $doc->id;
                    $temp = substr($id,47);
                    $url = urldecode($array[$temp]);
                ?>
            <a href="<?php echo $url; ?>" target="_blank">View the Detail Websit</a>
    ...
    <script>
        $(function() {
            var URL_PREFIX = "http://localhost:8983/solr/firstrank/suggest?q=";
